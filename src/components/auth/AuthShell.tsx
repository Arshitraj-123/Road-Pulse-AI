    ...
    <div className="min-h-screen bg-surface transition-colors dark:bg-[#0A1628]">
      ...
      >
        ...
        <motion.span
          ...
          className="mb-4 self-start rounded-full bg-teal-light px-3 py-1 font-mono text-[11px] uppercase tracking-wider text-teal-dark dark:bg-teal-mid/20 dark:text-teal-mid"
        >
          {eyebrow}
        </motion.span>
        <motion.div
...
export function AuthCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-sm transition-colors dark:border-white/10 dark:bg-white/[0.03]">
      {children}
    </div>
  );
}
